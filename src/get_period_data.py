...
def get_period_data(period, table_code, currency_code):
    ...
    for i in range(0, iterations):
        if i != iterations-1:
            ...
            url = str("http://api.nbp.pl/api/exchangerates/rates/{0}/{1}/{2}/{3}/?format=json".format(table_code, currency_code, period, temp_period))
            ...
            url = str("http://api.nbp.pl/api/exchangerates/rates/{0}/{1}/{2}/{3}/?format=json".format(table_code, currency_code, period, today))
            response = requests.get(url)
            if response.status_code != 200:
                print("Error: request returned code: " + str(response.status_code))
            else:
